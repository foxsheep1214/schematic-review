...
# -*- coding: utf-8 -*-
"""
自动扫描（方式 A）与证据计算（方式 E）——规则编号见 scripts/catalog.py
...
冷跑只执行自动扫描；提供 --evidence 的热跑再执行证据计算。
**扫描输出是疑似清单，不是判决。** 合法结构（Bob-Smith 终端、补偿网络、
...
def hot_rules():
    """全部证据计算规则（内置与检查器）；未执行的照样要列出来。"""
    return [(rule.id, rule.title) for rule in catalog.rules(method='E')]
...
class Lint:
    ...
    def run(self):
        ...
        for n, nds in nets.items():
            ...
            if len(nds) == 1:
                ref = nds[0].split('.')[0]
                self.add('NET-A01', '单节点悬空网',
                         f'{n} <- {nds[0]} ({parts.get(ref, {}).get("value", "")})', ref)

        # NET-A02 疑似网络名分裂（剔除同族总线/差分对/序号兄弟）
        names = sorted(nets)
        ...
        for n, nds in nets.items():
            ...
            if not any(r[0] in 'UJMY' for r in rs):
                self.add('NET-A03', '自动命名网仅含无源件', f'{n}: {sorted(rs)}')
        ...
        for node, pn in pinname.items():
            ...
            ref = node.split('.')[0]
            if n is None:
                self.add('PWR-A02', '电源球无网络', f'{node} ({pn})', ref)
            elif n not in self.pseudo and not self.driven(n):
                self.add('PWR-A02', '电源球所在轨无驱动', f'{node} ({pn}) <- {n}', ref)
        ...
        if not self.pintype:
            self.skipped.append(
                ('NET-A06', catalog.title('NET-A06'), '网表未提供 pintype'))
        else:
            coverage = len(self.pintype) / max(len(self.pin2net), 1)
            if coverage < 1.0:
                self.skipped.append((
                    'NET-A06', catalog.title('NET-A06'),
        ...
        for base, grp in rails.items():
            if len(grp) > 1:
                self.add('PWR-A04', '同基名多轨（确认非张冠李戴）',
                         f'{base}: {sorted(grp)}')

        # DOC-A03 BOM/库字段卫生；REQ-H01 改版闭环由 diff_netlists.py 负责
        for ref, v in parts.items():
            for field in ('part', 'value', 'jedec', 'prim'):
                value = v.get(field, '')
                if isinstance(value, str) and value != value.strip():
                    self.add(
                        'DOC-A03', 'BOM/库字段含首尾空白（影响比对）',
                        f'{ref}.{field}: {value!r}', ref)

        # REQ-A01 关键器件计数（冷跑·参数化：需第 0 步意图清单）
            ...
            for key, want in expect.items():
                got = sorted(r for r, v in parts.items() if not v.get('nc') and key.upper()
                             in (v.get('part', '') + ' ' + v.get('value', '') + ' '
                                 + v.get('prim', '')).upper())
                if len(got) != want:
                    self.add('REQ-A01', '关键器件计数不符',
                             f'{key}: 意图 {want} 实为 {len(got)}'
                             + (f' {got}' if got else ''))
        else:
            self.skipped.append(('REQ-A01', catalog.title('REQ-A01'), '意图中缺少器件计数目标 intent.expect'))

        # RST-A01 使能脚上下拉：网络名或引脚功能名命中；已有 RST-E01 证据的网络交证据计算
        covered_rule12_nets = {
            self.target_net(check) for check in self.evidence.get('checks', [])
            if check.get('rule') == 'RST-E01'
        }
        for n, nds in nets.items():
        ...
        # PRO-A02: model/name-derived voltage is a retrieval hint, not breakdown data.
    ...
    def run_hot(self):
        """执行已提供结构化证据的证据计算规则；未提供的继续保持 pending。"""
    ...
    def _hot_required_passive(self, check):
        ...
        if not candidates:
            self.add('SIG-E01', '未找到要求的直接电阻连接', detail,
        ...
        if len({x['other'] for x in candidates}) != 1:
            self.add('SIG-E01', '电阻连接多个电源域，需节点分析', detail,
                     kind='CANDIDATE', check_id=check['id'], citation=check['citation'])
            return
        if limits and any(x not in candidates for x in links):
            self.add('SIG-E01', '存在其他电阻支路，等效模型不完整', detail,
                     kind='CANDIDATE', check_id=check['id'], citation=check['citation'])
            return
        if not limits:
            self.record_pass('SIG-E01', check, detail,
                             scope='仅电阻装配及两端连接；电平、时序和阻值窗口未判定')
            return
        if any(x['ohm'] is None or x['tol'] is None or x['ohm'] <= 0 for x in candidates):
            self.add('SIG-E01', '阻值/公差不完整或含短路支路', detail,
                     kind='CANDIDATE', check_id=check['id'], citation=check['citation'])
            return
        ...
        if (equivalent['min'] < float(limits.get('min', float('-inf')))
            ...
            self.add('SIG-E01', '等效电阻窗口不满足要求', detail,
    ...
    def _hot_pin_bias(self, check):
        self._bias_result(check, 'RST-E01')
    ...
    def _hot_pin_map(self, check):
        ...
        if mismatches:
            self.add(
                'DEV-E01', '符号引脚映射与官方定义不符', detail, ref,
                check_id=check['id'], citation=check['citation'])
        else:
            self.record_pass('DEV-E01', check, detail, scope='仅 expected 列出的引脚及名称')
...
def main():
    ap = argparse.ArgumentParser(description='自动扫描与证据计算（扫描输出为疑似清单）')
    ap.add_argument('db', help='parse_netlist.py 产出的 db.json')
    ap.add_argument('--log', help='netlist.log（导出日志，含 No_connect 等免费证据）')
    ap.add_argument('--intent',
                    help='第 0 步意图清单 JSON（适用性发现与 REQ-A01 所需）')
    ap.add_argument('--evidence',
                    help='资料取证形成的结构化证据 JSON；提供后执行对应证据计算规则')
    ap.add_argument(
        '--datasheet-audit',
    ...
    ap.add_argument('--plan-json', help='单独写出逐项检查计划 JSON')
    ...
    # 分组键含 kind——同一条规则可同时产出 FINDING 与 CANDIDATE
    by = defaultdict(list)
    for f in F:
    ...
    for rid, name in hot_rules():
        if rid not in lint.hot_executed:
            print(f'  {rid:12s} {name:32s} 未提供对应的结构化证据')
