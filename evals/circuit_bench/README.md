...
| 检查轨道 | 当前覆盖 | 不能由本版结果推断 |
|---|---|---|
| 原生计算 | PWR-E01 分压/反馈、SIG-E01 等效上拉；独立 KCL 与容差角点作答案 | 所有稳压器、完整 I2C 合规、任意非线性网络 |
| 提供电压证据 | RST-E02 采样电平、RC 充放电、缺证与过期证据 | 工具自动生成瞬态模型或完整启动时序 |
| 挑战集 | 非串并联桥式反馈网络；独立参考答案已确定 | 当前工具已具备通用节点分析能力 |

规则编号见[规则总表](../../references/check-catalog.md)。2026-09-17 起评测协议改用总表编号
（`stimulus.py` 写入 PWR-E01/SIG-E01/RST-E02，`worker.py` 提交 evidence `schema_version` 2）；
