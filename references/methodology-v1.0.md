...
> 版本 V1.2（文件名为兼容既有引用保留） | 适用范围：以 Cadence/OrCAD 网表为主数据源的电路原理图连通性与设计合规审查；可迁移至任意具备文件读取与代码执行能力的 AI 平台（含 ChatGPT Code Interpreter）。
...
- **AC0 Automated Check（自动检查）**：机械、无歧义、可穷举的规则，用脚本（或 AI 按规则逐条推理）全量扫描。解决**覆盖率**——数百个网络、数百个电源球一个不漏。
- **ER1–ER7 Expert Review（专家审查）**（按执行序）：datasheet 核实、供电建图、链路追踪、最坏情况分析、平台合规、图形复核、器件与供应链。解决**正确性与风险**。
...
0.  意图对齐（设计意图问卷）
1.  数据解析（三件套 → 结构化索引）
2.  AC0 自动检查 —— Lint 冷跑（纯网表规则全量扫描）
3.  ER1 datasheet 核实 —— Abs Max / 强制条款 / 公式常量 / 引脚语义
    └ 回补 AC0 热跑（Rule-08 / Rule-09 / Rule-14 / Rule-16）
...
|---|---|---|---|
| L0 自动 Lint | **L0** | **AC0** | 明确为 Automated Check，分冷跑/热跑两趟 |
...
- **Rule-01～Rule-18**：规则身份，编号保持不变，不添加 `AC0-` 或 `ERx-` 前缀。

Rule-08/09/14/16 虽由 AC0 热跑执行，却需要 ER1 提供 datasheet 结论；这正是规则身份不能与检查模块绑定的原因。
...
AC0 中依赖 datasheet 的规则——Rule-08（参数验算）、Rule-09（必需上拉）、
Rule-14（符号引脚映射）、Rule-16（strap 强制条款）——在本层完成后回头补跑。
AC0 因此是**两趟**：冷跑（纯网表）→ ER1 → 热跑（补齐依赖项）。
...
1. 修复后重新导出网表 → 复跑 AC0 Lint + 修复项专项验证。
...
3. 让 ChatGPT 用 Code Interpreter 直接运行第 2 章解析代码与第 3 章 lint 代码，输出 AC0 清单。
