...
> **C 级隔离（全自动流程强制）**：C 级项不得作为推导 A/B 级结论的依据。信息补齐前，依赖它的所有下游结论一并降为 C 级——不确定性只能向下传播，**不得被逐级洗白成确定结论**。全流程无人在场时，这是防止 agent 用臆测填补缺口的唯一结构性约束。
...
3. 查不到 datasheet 的器件：标 C 级置信度，列入报告待索取清单，**严禁凭印象补参数**。
...
AC0 因此**不是一趟跑完**，而是按输入依赖分三档：**冷跑**（只吃 `db.json`）→ **冷跑·参数化**（加第 0 步意图清单 `--intent`，供 Rule-07）→ ER1 → **热跑**（加 datasheet 结论，补齐 Rule-08/09/14/16 并把 Rule-12 的候选定判）。
