> V1.6 电路判据以 SKILL.md、review-checklist.md、wca-formulas.md 和 datasheet-evidence-schema.md 为现行契约。身份核实先于参数计算；本长文中的历史案例不能直接变成通用门限。
...
**检查标识表示主执行顺序；身份核实先于参数计算。** AC0 是唯一的自动检查层，ER1–ER7 是需要工程判断的专家审查层。流程步骤（0～10）、检查标识（AC0/ER1–ER7）和规则身份（Rule-NN）彼此独立。
...
| Rule-13 | 防护窗口待核 | 型号与轨名仅生成候选，分别核对 VRWM/VBR/VC 和工况 | 不能从 VRWM 直接推导烧毁 |
...
电源追踪以 scripts/lint.py 的 power_path() 为准；按位号前缀猜驱动的旧摘录已移除。
...
| **强制条款原文**（`must be pulled down` / `must be left floating` / 内部默认态） | Rule-16、ER5 —— **原文用词必须保留**，并核对保证参数、额定值、适用条件及系统后果 |
...
逐上拉和主动驱动通路追踪两端供电域，跨轨先列候选。核对准确器件的推荐/绝对额定、
逻辑保证门限、Ioff、内部钳位及允许注入电流，分别计算正常、单域掉电、先后上电
与外部先供电状态。缺少同时上电保证不能直接证明反灌；某输入支持掉电隔离也不能
推广到全芯片其他引脚。跨隔离边界的连接另核其系统目的和器件隔离结构。

### 5.6 热与浪涌

- 电阻功耗：高压分压（>20V）每颗算 P=U²/R；泄放/放电电阻按满载功耗封装。
- 开关类器件（eFuse/负载开关）：分别计算导通损耗与线性限流 VDS·ID/SOA、故障时间和重试能量；结温实现另作 HANDOFF。
...
检测位置由目标量决定：输入存在/输入欠压通常检测源端；实际输出监控或稳压反馈可检测保护后/负载端。应在正常、保护动作、掉电和反向供电状态下验证检测是否仍满足目的。

**为什么在 ER2 判而不在 AC0**：判定要回答「这条轨是源还是保护后」，答案只存在于电源树里，而电源树是本层的产物。AC0 阶段只有网表，写不出不高误报的判别式。

**执行**：建完电源树后，对每个检测/反馈分压回答三问——① 中点接的是谁的哪个脚（ADC/OV/UV/SENSE/MON）；② 上臂另一端挂在电源树的哪个节点；③ 该节点相对被测对象是源侧还是保护后。再核对目标量与故障状态需求；位置与检测目的不符才形成发现项。
...
| 电路 | 公式/规则 | 验收标准 |
|---|---|---|
| 可调电源 FB 分压 | Vout=Vref×(1+Rtop/Rbot) | 含实际电阻公差、Vref 极限及输入偏置，窗口满足目标及负载推荐工作范围 |
...
| 稳压管钳位 | 按 I-V 曲线与源阻抗联立求解 | 宽输入范围下端高于检测门限、上端低于引脚耐压；低流拐点（knee）要计入 |
| ADC 检测分压 | Vadc=Vin×R2/(R1+R2) | 满量程不超 ADC 上限；各判定档位间隔 ≫LSB 与误差之和 |
| RC 复位 | 用 V(t) 门限穿越时间，含初值/源斜率/泄漏/公差 | τ 不等于保证脉宽 |
| I2C 上拉 | 按灌电流、上升时间与总线电容求等效上拉窗口 | 多器件地址不冲突；上拉电源与 GPIO 域一致 |
| MLCC 直流偏压 | 有效容量 = 标称 × 偏压系数 | 偏压系数取准确料号在实际电压/温度下的厂家曲线，不得低于功能需求 |
...
2. **只提取可判定条目**。判据：这条规则能否**仅凭网表或图纸就判定真/假**。
   - 可判定 →「XIN/XOUT 之间必须并联反馈电阻」「指定型号、封装和代际的控制器/DRAM 校准脚端接」「某 IO 域仅支持 1.8V」
   - 不可判定（属 Layout/软件，另行归档）→「去耦电容须靠近引脚」「DTS 需 disable 未用功能」
3. **逐条写成三元组**：`条件 → 期望值 → 判定方法（查哪张网/哪个位号/哪个字段）`。没有判定方法的条目不入库。
4. **标注强制等级**：区分 datasheet 里的 `must / shall`（强制，违反即 BLOCKER 候选）与 `recommended / typical`（建议）。**原文用词必须保留**，定级还需结合保证参数、额定值和系统后果；禁止只找 must 字样。
...
| 检测点接错侧 | 检测位置与目标量及故障状态要求不符 | Rule-11（ER2 §5.7） |
