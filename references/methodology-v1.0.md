...
- **L1–L7 专家审查层**（按执行序）：datasheet 核实、供电建图、链路追踪、最坏情况分析、平台合规、图形复核、器件与供应链。解决**正确性与风险**。
...
### 0.2.1 层号迁移对照（自 V1.0 起）

早期版本把「分类」与「顺序」混为一谈，导致 datasheet 核实排在最后、
读图排在第 8 步，而两者实际都是前置或贯穿项。V1.1 起层号即执行顺序：

| 旧层号 | 新层号 | 变化 |
|---|---|---|
| L0 自动 Lint | **L0** | 不变；明确分冷跑/热跑两趟 |
| L6 前半 datasheet 核实 | **L1** | **提到最前**——一切判断的前置 |
| L2 供电审计 | **L2** | 不变 |
| L1 链路追踪 | **L3** | 后移，先建图再走路径 |
| L3 参数验算 | **L4** | 顺延 |
| L4 平台规则 | **L5** | 顺延 |
| L5 后半 系统目检 | **L6** | 拆分 |
| L5 前半 按需渲染 | **贯穿** | 不再排队 |
...
| 平台设计规范（HDG/Checklist/PinOut） | L5 规则扫描依据 |
...
4. 再逐章指令执行 L1-L7（链路追踪、供电审计、WCA、规则扫描、读图、器件核实）。
