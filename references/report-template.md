...
## 定位与引用规范（2026-09-01 起强制）

- **发现项位置**：所有等级（致命/严重/建议/观察）发现项必须给出 `页码 + 位号 + 网络名`，
  建议/观察表格条目同样标页码；页码以网表 ref2page 与 PDF 实际页为准。
- **datasheet 引用**：凡引用参数/条款/真值表/公式/引脚语义，必须标注
  `文档名 + 版本 + 页码（或表号/章节号/图号）`，示例：`LM5069 SNVS452G §7.1 p.4`、
  `KSZ8863 DS00002335C p.9 strap 真值表`。禁止无定位的"datasheet 显示"。
- **网络补取 datasheet**：在「审查依据」单独成组列出（URL + 文档版本 + 获取日期），
  与资料包提供件区分；立创商城商品页来源注明商品 C 号。
