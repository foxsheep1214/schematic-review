...
新节点路径使用原 VALUE 十进制的精确分数与 LDLᵀ 求解，枚举每颗可变电阻、Vref 和偏置
端点的笛卡尔积；每颗共享电阻只计一次。结果保留精确 min/max 与极值角点，浮点展示向外
舍入，门限比较使用精确分数。其参考计算仅证明给定单源/单参考静态模型下的设定窗口。
0Ω、无法解析的电阻、未建模输入、非线性/多源或超出上限的网络继续 INSUFFICIENT。
非零地偏差、源供电余量、启动和环路稳定性仍需独立证据或经核实的仿真，不能由此放行。
