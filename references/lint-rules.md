...
| L0-15 | "NC" 网络实为真网络 | 存在名为 NC/NC_* 的多节点网络。**必须先判别真短路 vs 工具伪网络**，见下方「NC 网络判别式」 | 真短路：164 引脚被短成一张网，含晶振脚/MII 输出脚/6 个外部连接器 pin1（OrCAD 经典陷阱）。伪网络：357 引脚挂在 PSTWRITER 的 No-Connect 汇集网上，看似隔离栅被跨接，实为工具产物 |
...
`scripts/parse_netlist.py` 已自动完成第 1 步并把结果放进 `pseudo_nets`；`scripts/lint.py` 据此把伪网络降级为 `L0-15-INFO`，不再产生灾难级误报。

## 驱动源判定（L0-04/L0-05 前提）

判断"电源轨有无驱动"时，以下**五类全部**算驱动源。漏掉任何一类都会产生大批假"无驱动轨"——实测漏算 0R 与模组输出时，L0-05 从 1 条真命中膨胀到 44 条：
