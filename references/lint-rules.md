...
| **热跑** | `db.json` + ER1 的 `evidence.json` + `datasheet-audit.json` | Rule-08、09、12、14、16 |
...
| Rule-04 | 电源轨无驱动 | 从命名电源轨沿已装配通路追到可能电源，按状态核来源 | 跳线本身不是电源，受控路径仍需 ER2 验证 |
...
`scripts/checkers/` 里的检查器自带规则，编号用检查器前缀，**01–09 冷跑、10 起热跑**，
与 `Rule-NN` 并存且互不改写。它们同样进计划、进覆盖台账，未执行照样列在「本趟未执行」。
逐检查器的识别依据、intent 字段、缺口语义见 [checkers.md](checkers.md)。

| ID | 名称 | 检查逻辑 |
|---|---|---|
| IL-01 | 感性负载无续流/钳位路径 | 开关驱动的线圈/绕组在该装配状态下无续流二极管、TVS/齐纳、RC 或有据的集成钳位 |
| IL-02 | 续流二极管方向接反 | 续流二极管阴极在开关节点、阳极在电源轨 |
| PS-01 | 栅极无驱动源且无下拉 | 分立开关管栅极网上既无驱动输出脚/外接驱动，也无下拉/上拉电阻 |
| PS-02 | 开关节点无吸收/钳位 | 开关节点接感性元件或半桥对管，但无 RC/RCD/钳位（CANDIDATE） |
| IF-01 | 输入串联滤波无阻尼元件 | 变换器输入网经 L/磁珠滤波，但无 RC 阻尼支路且无带 ESR 的体电容（CANDIDATE） |
| PU-01 | 使能/复位与供电轨同时建立 | 器件的使能/复位输入与其自身供电脚同网（CANDIDATE） |
| PU-02 | 使能直连输入轨无 UVLO/延时 | 稳压器使能脚直接坐在自身输入网上，无分压或 RC（CANDIDATE） |
| PU-03 | 使能来源不确定 | 稳压器使能网上既无驱动源，也无分压、RC 或上/下拉，悬空或来源不明（CANDIDATE；器件内部上/下拉需资料证据） |
| SV-01 | 喂狗输入悬空或固定电平 | WDI 网悬空或直接坐在电源/地上（CANDIDATE，需确认是否有意禁用） |
| SV-02 | 复位/看门狗输出未到复位输入 | 输出网不含任何复位输入脚；网上无其他器件时为 FINDING，只接到其他脚时为 CANDIDATE |
| SV-03 | 存在未被监测的电源轨 | 已识别监控器的前提下，给 IC 供电的轨无 sense 脚也无到 sense 的分压（CANDIDATE） |
| DL-01 | 交流耦合发送端无直流通路 | LVPECL/PECL 类电平交流耦合，发送侧无到地/到轨的直流通路；声明依据出 FINDING，名称线索出 CANDIDATE |
| DL-02 | 交流耦合接收端无偏置/端接 | 接收侧交流耦合后既无端接也无偏置（CANDIDATE） |
| OC-01 | LED 回路无限流元件 | 光耦 LED 两条腿上都没有串联电阻，且未声明恒流驱动 |
| OC-02 | 输出集电极无上拉 | 光耦集电极网上无到电源轨的上拉电阻（CANDIDATE） |

热跑规则（走 `evidence.json`，字段见 [datasheet-evidence-schema.md](datasheet-evidence-schema.md)）：
PS-10 栅源驱动窗口、IF-10 负输入阻抗与阻尼一阶判据、PU-10 最坏压差裕量、
SV-10 复位脉宽与上拉、DL-10 共模与摆幅兼容、OC-10 CTR 与驱动能力。保证值缺一项即
INSUFFICIENT；比值/寿命系数必须来自项目规定。

单独另存某个检查器的清单：`python3 scripts/lint.py db.json --checker-json <id>=out.json`。
...
1. 取该网的 `C_SIGNAL` 与实例行，核对已验证的导出格式；无层次路径本身不能证明伪网。解析器只对满足 NC 命名约定的平面汇集网给出候选分类，普通平面网络仍参与电气检查。
...
`scripts/parse_netlist.py` 把启发式分类放进 `pseudo_nets`；`scripts/lint.py` 据此输出 `Rule-15-INFO`。Agent 仍须用图面/属性确认分类，不能用该字段自证。
...
`hot_executed` 只说明规则被调用；`hot_uncovered_instances` 列出计划中对象证据缺失、
过期或模型未就绪的热跑实例。仍需 Agent 复核计划完整性与最终结果；它不是全板覆盖证明。
Rule-12/16 用包含负载和采样时序的保证电压窗口比较门限；缺少模型时为 CANDIDATE，
不能拿上拉电源电压直接当分压后脚压。`check_results` 留存状态和判定范围。
