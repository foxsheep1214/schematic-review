...
> V1.2｜由四个真实工程项目的多轮审查闭环沉淀合并而成，所有规则均有实证出处。

## 核心思想

原理图的电气本质是**网络表**，图形只是可视化。审查不依赖 EDA 软件：把导出网表解析成结构化数据，分两层执行——

- **AC0 Automated Check（自动检查）**：机械、无歧义、可穷举的规则全量扫描，解决**覆盖率**（几百网络、几百电源球一个不漏）。输出是疑似清单，合法结构（Bob-Smith、DNP 选项）由执行 agent 逐条排除。
- **ER1–ER7 Expert Review（专家审查）**（按执行序）：datasheet 核实 → 供电建图 → 链路追踪 → 参数验算 → 平台规则 → 图形目检 → 供应链，解决**正确性与风险**。
...
| **C 待确认** | 信息不足或来源不可靠 | 在报告中成节列出待求证项，**严禁臆测** |

器件类别必须 ≥2 个独立证据源才允许定级 A/B（实例：浪涌器件凭库名误判为 TVS，原厂规格书证伪其为 MOSFET 结构件）。

**C 级隔离（全自动流程强制）**：C 级项不得作为推导 A/B 级结论的依据。信息补齐前，依赖它的所有下游结论一并降为 C 级——不确定性只能向下传播，**不得被逐级洗白成确定结论**。
...
### 第 0 步：意图对齐（必须先做）
**由执行 agent 从输入材料（需求/规格书、原理图标题页与修订记录、历史评审记录）自行提取并成文留痕，不得中断流程向人询问。** 需提取：板卡功能一句话；电源链路与功耗预算；必须保留/删除的接口清单；环境约束（温度/振动/防护/EMC）；降额策略；不可更改项（认证、绑定芯片、平台强制规则）。**材料中提取不到的条目逐条标 C 级「待确认」，并在报告头部声明「规格符合性判定受限」——不得凭空补全。**该意图清单是 Rule-07 与 ER5 的判定输入，必须随报告留档：它错了，下游结论会连锁错。
...
### 第 2 步：AC0 自动检查（全量 Lint）
`python3 scripts/lint.py db.json --log netlist.log [--intent intent.json]`。规则库与判别式见 `references/lint-rules.md`：单节点悬空网、双胞胎网络名、自动命名孤岛、电源轨/电源球无驱动、VSS 未入地、关键器件计数、ESD 挂残网、EN 极性/耐压、钳位管直连电源、NC 网络实为真网络、假闭环、同名不同域电源轨。**输出分两栏**：`FINDING` 逐条排除；`CANDIDATE` 是待 ER1 定夺的优先级清单，**直接决定下一步优先读哪几份 datasheet**。**两条前提**：① NC 网络必须先判别「真短路 vs 工具伪网络」，判反了要么漏真缺陷、要么报出灾难级误报；② 驱动源判定须涵盖 0R 跳线与模组自身输出，漏算会产生大批假「无驱动轨」。两者脚本已内置。**脚本末尾会列出本趟未执行的规则及原因——0 条 ≠ 通过，别把没扫读成扫过了。**
...
**回补 AC0（热跑）**：AC0 中依赖 datasheet 的规则（Rule-08 参数验算、Rule-09 必需上拉、Rule-12 EN 极性/耐压——冷跑已把候选列出、
Rule-14 符号引脚映射、Rule-16 strap 强制条款）在本层完成后回头补跑。

查不到 datasheet 的标 C 级并列入报告待索取清单，**严禁凭印象补参数**；网络聚合页信息一律不可靠。表格列错位时先渲染目检再取值——工厂固定档被误读成可调档，会直接产出假缺陷。
...
修复后重新导出网表 → 复跑 AC0 + 修复项专项验证 → 与上版 diff（BOM 变更+网络变更），确认"只改了该改的" → 致命/严重清零后输出终审结论与投板前目检清单。改版复审沿用同一框架：先跑新旧网表全量 diff，再对前轮问题逐条做闭环验证（实例：前轮 22 项问题表格化逐条判定 已修复/未闭环/未变）。
...
> **三个编号体系解耦**：步骤 0～10 只表示工作流位置；AC0/ER1–ER7 只表示检查模块与执行顺序；`Rule-01`～`Rule-18` 只表示规则身份，编号保持不变。
...
- 本 skill 的 scripts/、references/、examples/ 均为可执行资产，审查时按需取用，不必全量加载：
  ...
  - `scripts/lint.py`：AC0 自动检查全量扫描，内置伪网络判别与五类驱动源识别
