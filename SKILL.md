...
> V1.6｜按电路与状态检查 + 保证边界验算 + 证据依赖闸门 + AC0/ER1–ER7 + 网表 Diff。
...
**检查标识中的数字表示主执行顺序，器件身份先在 ER1 唯一化、ER7 再总复核**，且顺序本身是有依据的：先有 datasheet 才谈得上"违反"，先有电源树才有地图可走链路，先走通链路才知道该算哪些点。流程步骤（0～10）、检查标识（AC0/ER1–ER7）与规则身份（Rule-NN）是三个独立编号体系。
...
- 在参数代入前核对实际订货型号、封装、后缀/档位和装配状态；VALUE/PART/PRIM 是检索线索，冲突必须有明确的身份解释。ER7 做完整性复核，不把身份判定推迟到计算之后。
- 把适用功能拆成具体电路、位号/网络、装配版本、工作/故障状态和单个判据。由 agent 在 intent.circuits 中登记，再由 plan_review.py 展开；功能域只作覆盖父项，不能一次性 PASS。电路域与状态填写方式见 references/review-plan-schema.md。
- 热跑证据必须绑定当前网表指纹、准确物料及文档内容指纹；所依赖资料未 AVAILABLE 时，规划和执行均保持 WAITING_EVIDENCE / INSUFFICIENT。关键 L/F/Y/J/C/R 的额定值、曲线或引脚参数按需要加入资料审计，不能只查 IC。
- Rule-08 只自动求解已声明电源/参考端和输入负载的可归并电阻网络。公差/Vref 极限缺失、共享路径、未建模半导体、递归截断均不得 PASS。不同地网不能按名称视为短接。
- Rule-09 分开记录装配/两端连接、等效阻值与总线电气约束。Rule-12/16 必须使用包含输入泄漏、内部拉阻、外部负载和采样时序的电压窗口；该窗口由 agent 留存计算或仿真依据，脚本只比较保证门限，不代替瞬态求解。
- 每条结果限定状态、参数和范围；零命中、READY、资料齐全、单个电阻存在或某个样品能启动，均不能代替整项电气检查 PASS。计算、原始数据与保证条件的语义真实性仍需 ER1/ER4 复核。

结构化输入与迁移示例见 references/datasheet-evidence-schema.md；电气判据及原厂出处见
references/wca-formulas.md、references/review-checklist.md。旧 evidence.json 仍可读，缺新证据时会明确等待，禁止为消除等待而虚填指纹、零泄漏或零公差。
...
**检查标识表示主执行顺序；身份核实与图形复核可按依赖提前。** AC0 是唯一的自动检查层，ER1–ER7 是需要工程判断的专家审查层。
...
**资料包覆盖核对（第 1 步尾部机械执行）**：以 db.json 中全部非 NC 有源器件（U/M/Q/D
等 IC 类位号）对照资料包 datasheet 清单，产出《缺失 datasheet 清单》——它是 ER1 的
...
| **强制条款原文**（`must be pulled down` / `must be left floating` / 内部默认态），**原文用词必须保留** | Rule-16、ER5 —— 保留适用对象和条件；保证参数、额定值与系统后果也可构成阻断依据 |
...
**datasheet 补取协议（资料包缺失时，联网检索的规则）**：缺失清单中审查实际需要的
器件，允许联网补取 datasheet，五条规则——
1. **身份以 value 为准**：BOM 二义（part/prim ≠ value）时，以 value 字段的规格型号
   检索并核对文档（二义本身仍按发现项列出，但分析按 value 身份推进）。
2. **检索顺序与来源白名单**：**先查立创商城（LCSC）**——其商品页挂接的 datasheet
   多为原厂官方 PDF 镜像，可作为有效来源（商品 C 号随报告留痕）；再查原厂官网
   （ti.com、microchip.com、analog.com、st.com、sg-micro.com、gigadevice.com、
   jscj 等）。聚合页（alldatasheet 类）与转载一律不算数。**立创与公开渠道均找不到
   的，保持 review_result=INSUFFICIENT、evidence_confidence=C，明确列出并说明"由设计者提供"，不得硬补**。
3. **版本校验**：下载后核对文档覆盖的系列与实际订货号，并**对应订货表逐项匹配**（含后缀档位、温度
   等级、封装变体，如 -3.3 vs -ADJ、AEY vs AEV）；同系列不同档位不得混用；中英文
   版并存时优先英文官方版，记录文档版本号/日期。
4. **参数对撞**：补取 datasheet 用于支撑 A/B 定级的关键参数（Abs Max、强制条款、
   公式常量、引脚语义），尽量与一个独立迹象对撞（参考设计、demo 参数、封装丝印、
   同系列兄弟型号）。
5. **留痕**：报告「审查依据」分两类标注——资料包提供 / 网络补取（URL + 文档版本
   + 获取日期）；补取文件放 /tmp/codex-work/<task>/，不混入资料包。
...
### 第 4 步：ER2 供电审计
每轨核对来源、负载及电压保证范围；0R/磁珠/电感要追另一端。电源球的实际导通路径与 VSS 的地网归属逐项检查，不能只凭名称或元件存在。功率预算使用有来源的最大负载、温度/输入极限和适用降额要求，缺负载电流则 INSUFFICIENT。跨轨上拉先列候选，再逐端核对 VIH/VIL、VOH/VOL、推荐/绝对额定、Ioff、钳位和允许注入电流，覆盖先后上电、单域掉电及外部先供电；不能由轨不同或缺少同时上电保证直接断言倒灌。跨隔离域需另核电气隔离目的。损耗区分稳态 I²R、开关损耗和线性热插拔 VDS·ID/SOA。

本层只判断原理图可证明的拓扑、逻辑时序和电气应力。PCB 压降、PDN、布局后的
实际热路径不在本层定判；把大电流、去耦和散热要求写入独立 handoff。

**Rule-11 检测点选错轨在本层判**（已从 AC0 迁入）：先声明检测目的：输入存在/输入欠压取源侧，实际输出检测或稳压反馈可以取保护后或负载端。判定要回答「要检测哪个量、保护动作后还需检测什么」，结合电源树及故障状态判断——AC0 阶段无此上下文（实例：VBUS 判压分压错接输出侧 VOUT_24V）。详见 `references/methodology-v1.0.md` §5.7。
...
**报告定位与引用规范（2026-09-01 起强制执行）**
1. **证据链引用 datasheet 必须可定位**：凡发现项/结论引用 datasheet 内容（参数、
   条款、真值表、公式、引脚语义），标注 文档名+版本+页码（或表号/章节号/图号），
   示例：`LM5069 SNVS452G §7.1 p.4`、`KSZ8863 DS00002335C p.9 strap 真值表`、
   `STM32H743 DocID030538 Rev3 p.95 Figure 13`。禁止无定位的"datasheet 显示"。
   网络补取的 datasheet 另需标注来源 URL 与获取日期。
2. **全等级发现项必须带原理图位置**：致命/严重/建议/观察各档发现项一律给出
   **页码 + 位号 + 相关网络名**（页码以网表 ref2page 与 PDF 实际页为准）；
   建议/观察类表格条目同样标注页码，不得只写器件类别或功能域。
...
## 定级校准与经验归档（2026-08-31 STD100 复审）

每轮审查与高频错误模式库一并强制自查；以下均为真实误判归档，对应报告附录 D。

- [ ] **数字输入保证边界**：对要求 high 的采样窗口，最坏电压必须 ≥ VIH(min)；要求 low 则必须 ≤ VIL(max)。已知窗口不满足保证条件应判该检查项 FAIL，严重度按系统后果确定，关键启动配置可阻断准出。落在不定区不能断言每颗样品必然失败，也不能自动降为建议；缺门限、泄漏或采样时序则为 INSUFFICIENT。
- [ ] **后果级联逐环标概率**：致命定级要求级联链每环近确定；任何一环未知（如固件行为、内部上拉阻值）→ 整链降级 + 列 C 级求证项，禁止把未验证假设连乘成致命（实例：strap→SPI 不可达→Reg198 写不了→RMII 死，第二环依赖未知假设）。
- [ ] **实测适用范围**：bench 可证实某样品、温度、供电与时序下的表现，也可发现模型错误；单次正常结果不能覆盖工艺/电压/温度/批次保证边界。实测与推算冲突时复核输入、模型和条件，分别记录样品表现与设计保证性。
- [ ] **置信度不越层**：事实层（阻值/门限）的 A/B 置信度不得带到结论层（会读 1/功能会死）；结论链依赖未知输入时按 C 级隔离，不确定性只能向下传播，不得逐级洗白。
- [ ] **图形证据定级门槛**：二极管/TVS/极性类方向判读若用于支撑严重及以上定级，必须 ≥400dpi 渲染复核（实例：200dpi 误读 SS56L 方向为 VS→VOUT 判"跨接致命"，400dpi 证实为 VOUT→VS 掉电保护，方向判反）。
- [ ] **No_connect 属性残留（ORCAP-36038）处置流程**：已连线引脚仍带 NC 属性（符号库引脚属性残留，或 × 被压在标签/连线下方）——图面通常不可见、导出器以连线为准、**不影响 Layout 与单板可制造性**。标准流程：网表定位 → 原理图 PDF ≥400dpi 放大交互印证 → 确认连线正确后**定级最低档（观察）**，仅建议顺手清理以保持导出日志与库卫生（实例：U12.E17/J12.1/J16.1 属性残留，400dpi 验证连线正确）。
- [ ] **检索不能只看网络名**：端接/阻尼电阻可能藏在未命名中间网（实例：RS485 端接 R317 在 PTC 内侧中间网，按 "RS485" 检索漏检，险报"无端接"假缺陷）——查端接类元件须对链路网逐成员展开。
- [ ] **工具输出先对撞再引用**：分压求解器等脚本输出必须与轨名/独立信息源对撞，对不上先怀疑脚本（实例：RAIL_RE 误匹配 FB 网名 `VOUT_FB_1` 产出 1.54V 伪解，手工复核真实值 12.27V）。

## 工作纪律

- 结论可被质疑：每条发现给出可复现核验路径（文件:行号/页码/表号）；被反驳时用证据复核，错了明确撤回并归档教训
- 更正前轮（含自己）结论写成"更正说明"，不悄悄改
- 报告写入项目目录；中间产物（页面文本、渲染图、底稿）放 /tmp/codex-work/<task>/；交付物只有报告本身
...
- 本 skill 的 scripts/、references/、examples/ 均为可执行资产，审查时按需取用，不必全量加载：
  ...
  - `scripts/solve_dividers.py`：有限拓扑的分压探索计算；热跑另核源端、负载和保证公差
  - `scripts/electrical_contract.py`：网表/文档指纹与共享证据依赖验证
