...
> V3.1｜规则总表（内容域 × 检查方式，编号稳定）、功能包展开、需求追溯、对象覆盖、工况审查、严重度校准、面向新手的修改步骤与结果校验。
...
网表检出或意图声明的功能包（总表“功能包”）展开全部成员规则：把实际电路和适用状态填入
...
逻辑电平与驱动（SIG-C07）、时序裕量（SIG-C09）和敏感负载的纹波预算（PWR-C14）按最坏角点计算。
