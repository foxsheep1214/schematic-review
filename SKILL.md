...
- **AC0 Automated Check（自动检查）**：机械、无歧义、可穷举的规则全量扫描，解决**覆盖率**（几百网络、几百电源球一个不漏）。输出是疑似清单，合法结构（Bob-Smith、DNP 选项）人工排除。
...
**回补 AC0**：AC0 中依赖 datasheet 的规则（Rule-08 参数验算、Rule-09 必需上拉、
