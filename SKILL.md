...
2. **检索顺序与来源白名单**：**先查立创商城（LCSC）**——其商品页挂接的 datasheet
   ...
   的，保持 INSUFFICIENT/C 级，明确列出并说明"由设计者提供"，不得硬补**。
3. **版本校验**：下载后先核对文档抬头型号与 value **逐字一致**（含后缀档位、温度
...
- [ ] **数字输入三态区模型**：VIL(max)/VIH(min) 是 datasheet 保证边界而非判决点；采样点落在不定区（如 0.8~2.0V）只证明"不保证 0/1"，属裕量问题（建议级）。定"strap/输入失效"必须与典型翻转点或实测数据比对；datasheet 只给保证边界时禁止升级为功能失效（实例：KSZ8863 P2LED0 采样 0.9~1.2V 初判 SPI 必死，实物网口已调通证伪）。
- [ ] **后果级联逐环标概率**：致命定级要求级联链每环近确定；任何一环未知（如固件行为、内部上拉阻值）→ 整链降级 + 列 C 级求证项，禁止把未验证假设连乘成致命（实例：strap→SPI 不可达→Reg198 写不了→RMII 死，第二环依赖未知假设）。
- [ ] **结果层 bench 证据优先**：参数采信序是 datasheet 原文 > 网表实测 > 页面注释；但**结果**层面实物 bench 证据高于模型推算——下"失效"结论前必须先问"实物表现是什么"。
