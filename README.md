...
本项目同时保留逐物料资料补取、文档/网表指纹绑定、逐电路/状态计划及保证边界电气检查。
...
在项目审查目录持久保存输入清单和产物，命令中的 scripts 路径按 skill 位置调整：
    ...
    # Agent 核对/补取资料并写出 datasheet-resolution.json 后，重跑审计：
...
按 [资料补取契约](references/datasheet-resolution-schema.md) 完成 audit 的逐物料请求，
再按 [热跑证据契约](references/datasheet-evidence-schema.md) 绑定准确物料、文档和当前网表。
未绑定或资料不可用的旧证据仍可读取，但相关检查保持 WAITING_EVIDENCE / INSUFFICIENT。
...
旧 evidence/intent/diff 输入保持 schema_version=1（可读取旧结构）；最终结果为 v2。
热跑的 basis、文档指纹和模型边界是定判前提，缺失时不会转为 PASS。
...
| [datasheet-resolution-schema](references/datasheet-resolution-schema.md) | 逐物料资料审计、补取和依赖准备度 |
